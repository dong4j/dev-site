...
description: "通过脚本和 Makefile 自动化 Hexo 博客发布流程，包括图片处理、格式转换、图床上传和部署。"
pubDate: "2024-12-30"
category: "share"
banner: "./hexo-deploy-workflow/cover.webp"
tags:
  - "Hexo"
  - "Git"
  - "效率工具与工作流"
...
![cover.webp](./hexo-deploy-workflow/cover.webp)
...
自动为博客添加 tags 和 AI 摘要, 这个可以看 [另一篇博客](/posts/generate-blog-tags-using-ai/).
...
```yaml
skip_render: 
  # 忽略 年份目录 中所有文件, 这样只会处理 _posts/publish/**
...
在 [HomeLab存储与备份：数据堡垒-保障数据和隐私的存储解决方案](/posts/home-data/) 中有讲到如何使用 **3-2-1 备份原则** 来指导如何备份重要文件.
...
因为一开始提交了大量图片, 导致触发了 Gitee 的仓库容量限制阈值, 解决办法可以看 [解决 git 仓库体积过大导致 push 失败的问题](/posts/git-clean/).
