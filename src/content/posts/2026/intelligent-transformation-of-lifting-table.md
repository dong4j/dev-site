...
![20260818003651_zVY9sccH.webp](./intelligent-transformation-of-lifting-table/20260818003651_zVY9sccH.webp)

这个桌子从搬入新家就一直陪伴着我, 它承载了太多太多....

是的, 就是字面上的意思:
![20260818003651_saZITnHG.webp](./intelligent-transformation-of-lifting-table/20260818003651_saZITnHG.webp)
...
![20260818003652_iR2BviFd.webp](./intelligent-transformation-of-lifting-table/20260818003652_iR2BviFd.webp)
...
![20260818003652_4WBxX7JW.webp](./intelligent-transformation-of-lifting-table/20260818003652_4WBxX7JW.webp)
...
![20260818003652_cqo3fidq.webp](./intelligent-transformation-of-lifting-table/20260818003652_cqo3fidq.webp)

这套架构的核心是 ESP32-S3，它当控制中枢。北向对接 App、手表、键盘、旋钮这类控制端，设备只发统一指令，不用懂原厂协议。南向对接升降桌控制盒，由 ESP32-S3 把这些指令翻译成控制盒能识别的键码，桌子才会动。

硬件架构:
![20260818003652_e7FKWmHs.webp](./intelligent-transformation-of-lifting-table/20260818003652_e7FKWmHs.webp)

所以整套方案里，最关键的是拿到原厂控制盒认的那套指令。当前分层和硬件拓扑记在仓库的[架构总览](https://github.com/dong4j/desk-gateway/blob/main/docs/architecture/overview.md)里。
...
![20260818003653_fnxzHBKG.webp](./intelligent-transformation-of-lifting-table/20260818003653_fnxzHBKG.webp)

### 3.2 TM1650 芯片与总线类型判断

原厂面板使用 TM1650 驱动数码管, 结合 `CLK` 和 `DAT` 两个测试点，初步判断面板与控制盒之间使用 I²C 通信。

![20260818003653_Ek504MoF.webp](./intelligent-transformation-of-lifting-table/20260818003653_Ek504MoF.webp)

芯片型号还不能确定完整协议。只能继续通过总线抓包确认主从关系、设备地址、按键编码和高度数据。

### 3.3 逻辑分析仪接线与采样参数调整

![20260818003653_K0QNidXJ.webp](./intelligent-transformation-of-lifting-table/20260818003653_K0QNidXJ.webp)

抓包时，原厂面板保持与控制盒连接，逻辑分析仪接入 `GND`、`CLK` 和 `DAT` 三个测试点。分析仪不连接面板的 3.3V，也不参与供电。

![20260818003653_4VLZLjeF.webp](./intelligent-transformation-of-lifting-table/20260818003653_4VLZLjeF.webp)

我最初把采样率设为 20 kHz，只能看到电平变化，无法解码完整数据。把采样率提高到 12 MHz 后，PulseView 可以稳定识别地址和数据。

![20260818003654_4pIsU6Tw.webp](./intelligent-transformation-of-lifting-table/20260818003654_4pIsU6Tw.webp)
...
![20260818003654_3gONT6Fu.webp](./intelligent-transformation-of-lifting-table/20260818003654_3gONT6Fu.webp)
...
![20260818003654_G7ecsw62.webp](./intelligent-transformation-of-lifting-table/20260818003654_G7ecsw62.webp)
...
![20260818003655_0KwoTuNE.webp](./intelligent-transformation-of-lifting-table/20260818003655_0KwoTuNE.webp)
...
控桌不是 ESP32 主动发一条「升桌帧」，而是控制盒每大约 3.7 ms 来读一次，Slave 把当前键码交出去。固件在 [mxtark.c](https://github.com/dong4j/desk-gateway/blob/main/firmware/desk-gateway/components/drivers/mxtark/mxtark.c) 里大致是这样：
...
![20260818003655_NnINBsZk.webp](./intelligent-transformation-of-lifting-table/20260818003655_NnINBsZk.webp)
...
![20260818003656_KvnaePSX.webp](./intelligent-transformation-of-lifting-table/20260818003656_KvnaePSX.webp)

ESP32-S3 继续使用 USB-C 单独供电，并与控制盒共地。增加上拉电阻后，Web 页面可以控制桌子升高和降低。

接线图如下:

![20260818003656_FlVQqtOM.webp](./intelligent-transformation-of-lifting-table/20260818003656_FlVQqtOM.webp)
...
这不是把左右 RJ45 的 CLK/DAT 短接过去。短接会绕过 ESP32，童锁和来源权限都拦不住。面板按键从右口进 GPIO6 / GPIO7，和 Web、BLE 一样先交给 `desk_core`，再由左口 GPIO4 / GPIO5 的硬件 I²C Slave 回给控制盒。

这张图是数据怎么走：

![20260818003656_9ALH78Oi.webp](./intelligent-transformation-of-lifting-table/20260818003656_9ALH78Oi.webp)
...
![20260818003657_ncuVr4jU.webp](./intelligent-transformation-of-lifting-table/20260818003657_ncuVr4jU.webp)

左口对控制盒还是硬件 I²C Slave，地址 `0x24`。右口要对着原厂面板说话：ESP32 得装成控制盒，去问面板现在按的是哪颗键。抓包里这套问答大约 9.6 kHz，写完一笔、读键码之前还有一次 STOP。ESP-IDF 自带的硬件 I²C Master 会把写和读并成一笔，结束方式也对不上，复放不了这套时序。
...
ESP32-S3 用硬件 I²C Slave 顶替原厂控制面板，只应答控制盒对 `0x24` 的键轮询。业务代码更新当前键码，ACK、移位和总线时序都交给 I²C 外设。
...
![20260818003657_ubt1Q59I.webp](./intelligent-transformation-of-lifting-table/20260818003657_ubt1Q59I.webp)

产品路径已经占用 `@0x24` 回键码。同一组 GPIO4/5 上再去 ACK `0x34–0x37`，硬件外设做不到。

于是出现两条实验路：

**GPIO 边沿嗅探:** 硬件 Slave 继续只应答 `0x24`，另用 GPIO ISR 听 CLK/DAT，试图在不 ACK 的情况下偷看 digit 写。真机结果是 NO-GO：SCL 每个上升沿和 SDA 边沿都会打高频中断，和硬件 Slave 抢同一组脚。控制盒在 `0x34–0x37` 被 NACK 后往往会停写，纯监听拿不到段码 data。更糟的是升降本身开始不稳，高度仍然未知。
...
![20260818003658_r87jPer6.webp](./intelligent-transformation-of-lifting-table/20260818003658_r87jPer6.webp)
...
![20260818003658_Cy85nnuy.webp](./intelligent-transformation-of-lifting-table/20260818003658_Cy85nnuy.webp)
...
![20260818003658_fMbFEBOr.webp](./intelligent-transformation-of-lifting-table/20260818003658_fMbFEBOr.webp)
...
![20260818003659_Ugn1EH2W.webp](./intelligent-transformation-of-lifting-table/20260818003659_Ugn1EH2W.webp)
...
![20260818003659_lZ1WhbaM.webp](./intelligent-transformation-of-lifting-table/20260818003659_lZ1WhbaM.webp)
...
![20260818003700_mnNJlfuq.webp](./intelligent-transformation-of-lifting-table/20260818003700_mnNJlfuq.webp)
...
控制盒能听 ESP32 说话之后，我先做了局域网 Web。

![20260818003700_UDC4XjBR.webp](./intelligent-transformation-of-lifting-table/20260818003700_UDC4XjBR.webp)
...
![20260818003700_Nbr9kN5E.webp](./intelligent-transformation-of-lifting-table/20260818003700_Nbr9kN5E.webp)
...
现在可以全部使用键盘和旋钮来控制升降:
![20260818003701_KRnjTdMl.webp](./intelligent-transformation-of-lifting-table/20260818003701_KRnjTdMl.webp)
...
![20260818003702_xlUUAIsF.webp](./intelligent-transformation-of-lifting-table/20260818003702_xlUUAIsF.webp)
...
![20260818003702_2mMGcni4.webp](./intelligent-transformation-of-lifting-table/20260818003702_2mMGcni4.webp)
...
![20260818003702_14umJy8Z.webp](./intelligent-transformation-of-lifting-table/20260818003702_14umJy8Z.webp)
可以将听到的话直接映射成已经写好的 Shell 命令。命令和键盘、旋钮一样，调用 `scripts/desk-preset.sh`。提示词和命令表见[GoatRemote 集成](https://github.com/dong4j/desk-gateway/blob/main/integrations/goatremote/README.zh-CN.md)。

![20260818003703_kMrubID6.webp](./intelligent-transformation-of-lifting-table/20260818003703_kMrubID6.webp)
...
![20260818003703_tjzTc7Q6.webp](./intelligent-transformation-of-lifting-table/20260818003703_tjzTc7Q6.webp)
...
![20260818003703_9umo0dbt.webp](./intelligent-transformation-of-lifting-table/20260818003703_9umo0dbt.webp)
...
![20260818003704_TrDIuUWB.webp](./intelligent-transformation-of-lifting-table/20260818003704_TrDIuUWB.webp)
...
![20260818003704_WI5GllF6.webp](./intelligent-transformation-of-lifting-table/20260818003704_WI5GllF6.webp)
...
这里还添加了一个自动童锁的功能, 目的在我离家后自动打开童锁功能.
...
![20260818003704_PPBFrFQ9.webp](./intelligent-transformation-of-lifting-table/20260818003704_PPBFrFQ9.webp)
...
![20260818003705_6Mm66wpc.webp](./intelligent-transformation-of-lifting-table/20260818003705_6Mm66wpc.webp)
...
总体来说，目前这个项目已经达到我的预期了，后续有机会再来集成其他协议的升降桌。

下一步可能就是改我的键盘了：
![20260818003705_MhMy5120.webp](./intelligent-transformation-of-lifting-table/20260818003705_MhMy5120.webp)

接上 iPhone7P 的线性马达来实现打字震动的效果 😎.

目前遇到的问题是并联 3 个线性马达后震动就非常弱了, 怀疑是电流太小的问题, 有哪位大佬可以指定一下呀, 感谢 🙏🏻.
