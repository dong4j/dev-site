...
description: "HomeLab 网络架构设计指南，涵盖网络设备选择、安全策略和异地组网技术。"
pubDate: "2020-04-12"
category: "homelab"
banner: "./homelab-network/cover.webp"
tags:
  - "Homelab"
  - "学习笔记"
toc: true
...
![平面图.drawio.webp](./homelab-network/平面图.drawio.webp)
...
![弱电箱.drawio.webp](./homelab-network/弱电箱.drawio.webp)
...
![network.drawio.webp](./homelab-network/network.drawio.webp)
...
![SSH-LocalForward.drawio.webp](./homelab-network/SSH-LocalForward.drawio.webp)
...
![SSH-RemoteForward.drawio.webp](./homelab-network/SSH-RemoteForward.drawio.webp)
...
![SSH-multi-stage.drawio.webp](./homelab-network/SSH-multi-stage.drawio.webp)
...
![NPM-HTTP.drawio.webp](./homelab-network/NPM-HTTP.drawio.webp)
...
![NPM-HTTPS.drawio.webp](./homelab-network/NPM-HTTPS.drawio.webp)
...
![NPM_Surge_proxy.drawio.webp](./homelab-network/NPM_Surge_proxy.drawio.webp)
...
![auto-switch-hosts.drawio.webp](./homelab-network/auto-switch-hosts.drawio.webp)
...
在开始这章之前, 有必要了解一下 [NAT (Network Address Translation)](/posts/nat-guide/) <!-- 链接站内文章, 使用文件名, 不要后缀 -->
...
上面也提到了, 当通信双方都在 NAT 后面的时候, 直接连接不太现实, 因为大多数 NAT 路由器对源端口的随机化相当严格, 不可能提前为双方协调一个固定开放的端口. 必须使用一个信令服务器 (`STUN `, [自建 STUN Server?](/posts/nat-guide/)), 它会在中间沟通分配给对方哪些随机源端口. 通信双方都会和公共信令服务器进行初始连接, 然后它记录下随机的源端口, 并将其返回给客户端.
...
![ubuntu-wireguard.drawio.webp](./homelab-network/ubuntu-wireguard.drawio.webp)
...
![ubuntu-wireguard-double.drawio.webp](./homelab-network/ubuntu-wireguard-double.drawio.webp)
...
![wireguard-nat1.drawio.webp](./homelab-network/wireguard-nat1.drawio.webp)
...
![wireguard_relay.drawio.webp](./homelab-network/wireguard_relay.drawio.webp)
...
看过 [NAT (Network Address Translation)](/posts/nat-guide/) 之后肯定清楚打洞的原理, 只要隧道被打通后, 在 NAT 记录失效之前(UDP 约 30 秒)更新 **WireGuard** 的配置并建立连接就可以直接通信, 实现细节可以参考这篇博客: [WireGuard Endpoint Discovery and NAT Traversal using DNS-SD](https://www.jordanwhited.com/posts/wireguard-endpoint-discovery-nat-traversal/), 很有启发意义.
